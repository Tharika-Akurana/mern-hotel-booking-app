...
const GuestHouses = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const guestHouseListings = data.filter(listing => listing.hotelType === 'guestHouse');
        guestHouseListings.forEach(listing => {
          listing.samllDescription = summarizeDescription(listing.description);
        });
        setListings(guestHouseListings);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-thin underline text-black font-serif text-center mb-10">Guset Houses</h1>

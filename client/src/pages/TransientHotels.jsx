...
const TransientHotels = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const transientListings = data.filter(listing => listing.hotelType === 'transient');
        transientListings.forEach(listing => {
          listing.samllDescription = summarizeDescription(listing.description);
        });
        setListings(transientListings);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-thin underline text-black font-serif text-center mb-10">Transient Hotels</h1>
...
export default TransientHotels;

...
const BoutiqueHotels = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const boutiqueListings = data.filter(listing => listing.hotelType === 'boutique');
        boutiqueListings.forEach(listing => {
          listing.samllDescription = summarizeDescription(listing.description);
        });
        setListings(boutiqueListings);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-thin underline text-black font-serif text-center mb-10">Boutique Hotels</h1>

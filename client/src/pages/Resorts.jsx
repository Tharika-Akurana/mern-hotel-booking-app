...
const Resorts = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        const resortListings = data.filter(listing => listing.hotelType === 'resort');
        resortListings.forEach(listing => {
          listing.samllDescription = summarizeDescription(listing.description);
        });
        setListings(resortListings);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-thin underline text-black font-serif text-center mb-10">Resorts</h1>
